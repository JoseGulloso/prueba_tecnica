...
if (window.location.pathname.includes("bandeja-casos")) {
  ...
  $(document).ready(function () {
    ...
    function renderMetrics(casos) {
      ...
      const slaVencido = casos.filter(
        (c) => normalizeSemaforo(c.semaforo, c.estado) === "vencido",
      ).length;
      const proxVencer = casos.filter(
        (c) => normalizeSemaforo(c.semaforo, c.estado) === "proximo-a-vencer",
      ).length;
    ...
    $(
      "#filterEstado, #filterTipo, #filterPrioridad, #filterServicio, #filterResponsable, #filterSlaVencidos, #filterSlaProximos",
    ...
    $("#btnLimpiarFiltros").on("click", function () {
      $("#searchInput").val("");
      $(
        "#filterEstado, #filterTipo, #filterPrioridad, #filterServicio, #filterResponsable",
      ).val("");
    ...
    function renderTable() {
      ...
      $("#tableInfo").text(
        `Mostrando ${total ? start + 1 : 0}–${end} de ${total} casos`,
      );
    ...
    function renderPagination(total) {
      ...
      if (from > 1)
        html += `<li class="page-item"><a class="page-link" href="#" data-page="1">1</a></li>${from > 2 ? '<li class="page-item disabled"><span class="page-link">…</span></li>' : ""}`;
    ...
    $(document).on("click", "#pagination .page-link", function (e) {
      ...
      renderTable();
      $("html,body").animate(
        { scrollTop: $("#casosTable").offset().top - 80 },
        200,
      );
    });
    ...
    $("#btnExportar").on("click", function () {
      ...
      const rows = filteredCasos.map((c) =>
        [
          ...
          c.prioridad,
          c.estado,
          formatDate(c.fechaLimite),
          getSemaforoLabel(c.semaforo, c.estado),
        ]
