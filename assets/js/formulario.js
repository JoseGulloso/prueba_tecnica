...
$(document).ready(function () {
  ...
  loadJSON("../data/catalogo-fpqrs.json").done(function (data) {
    ...
    populateSelect(
      $("#fpqrsServicio"),
      catalogo.servicios.map((s) => s.nombre),
    );
  });
  ...
  function populateSelect($select, options, defaultText) {
    if (defaultText !== undefined) {
      ...
      const $placeholder = $select.find("option:first").detach();
  ...
  $("#fpqrsServicio").on("change", function () {
    ...
    resetSelect($("#fpqrsCategoria"));
    resetSelect($("#fpqrsSubcategoria"));
    ...
    if (servicio && catalogo) {
      ...
      if (entry) {
        populateSelect(
          $("#fpqrsCategoria"),
          entry.categorias.map((c) => c.nombre),
  ...
  $dropZone.on("keypress", function (e) {
    if (e.which === 13 || e.which === 32) $fileInput.trigger("click");
  });
  ...
  function handleFiles(files) {
    ...
    Array.from(files).forEach((file) => {
      ...
      if (
        ...
        errors.push(
          `"${file.name}": tipo no permitido. Solo PDF, JPG, PNG, DOCX.`,
        );
        return;
      }
  ...
  $("#fpqrsForm").on("submit", function (e) {
    ...
    $btn
      .prop("disabled", true)
      .html(
        '<span class="spinner-border spinner-border-sm me-2"></span>Radicando caso…',
      );
    ...
    setTimeout(() => {
      ...
      $btn
        .prop("disabled", false)
        .html('<i class="bi bi-pencil-square me-2"></i>Radicar caso FPQRS');
      ...
      historial: [
        {
          ...
          accion: "Caso radicado por portal ciudadano",
          usuario: "Sistema",
        },
      ],
    };
