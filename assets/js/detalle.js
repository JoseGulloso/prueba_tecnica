/**
 * detalle.js — Lógica del detalle del caso
  ...
  $(document).ready(function () {
    ...
    function renderComentarios(comentarios) {
      ...
      const html = comentarios.map(com => {
        const initials = com.usuario.split(' ').slice(0,2).map(w => w[0]).join('').toUpperCase();
        return `
    ...
    function renderAdjuntos(adjuntos) {
      ...
      const html = adjuntos.map(a => {
        ...
        let colorClass = '';
        if (a.tipo === 'application/pdf')  { iconClass = 'bi-file-earmark-pdf';  colorClass = 'pdf'; }
        else if (a.tipo?.startsWith('image')) { iconClass = 'bi-file-earmark-image'; colorClass = 'img'; }
        else if (a.tipo?.includes('word'))    { iconClass = 'bi-file-earmark-word';  colorClass = 'docx'; }
        return `
    ...
    function renderHistorial(historial) {
      ...
      if (!historial.length) {
        $('#listaHistorial').html('<p class="text-muted text-center">Sin historial.</p>');
        return;
      }
      const html = [...historial].reverse().map(h => `
    ...
    $('#btnConfirmarEstado').on('click', function () {
      ...
      if (!nuevoEstado) { $('#nuevoEstado').addClass('is-invalid'); return; }
    ...
    $('#btnConfirmarPrioridad').on('click', function () {
      ...
      if (!nuevaPrioridad) { $('#nuevaPrioridad').addClass('is-invalid'); return; }
    ...
    $('#btnConfirmarReasignar').on('click', function () {
      ...
      if (!nuevoResp) { $('#nuevoResponsable').addClass('is-invalid'); return; }
    ...
    $('#textoObservacion').on('input', function () { $(this).removeClass('is-invalid'); });
    ...
    $('#cierreObservacion').on('input', function () { $(this).removeClass('is-invalid'); });
    ...
    /* -------------------------------------------------------
       ...
       ------------------------------------------------------- */
    $(document).on('click', '.btn-outline-secondary[title="Descargar (simulado)"]', function () {
